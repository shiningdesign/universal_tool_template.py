...
'''
v008.4.3: 2017.03.12
  * add support for user definable 
v008.4.2: 2017.03.06
  * add ext option for quickFileAsk
v008.4: 2017.02.22
  * add nuke parent window, so it stays on top of nuke
v008.3: 2017.02.20
  * auto template builder compatiblility
v008.2.2: 2017.02.14
  * fix language update
...
import re # for window pattern search
import ctypes # windows window detections
    ...
    def qui(self, ui_list_string, parentObject_string='', opt=''):
        ...
        for i in range(len(ui_list)):
            if ui_list[i] in self.uiList:
                ...
                if uiType in self.qui_user_dict:
                    # first, try user dict for type short name
                    uiType = self.qui_user_dict[uiType]
                elif uiType in self.qui_core_dict:
                    # then, try default dict for type short name
                    uiType = self.qui_core_dict[uiType]
...
def main(mode=0):
    ...
    if hostMode in ("desktop", "blender"):
        ...
        #winTitle = self.name + " - v" + self.version + " - host: " + hostMode
        is_opened = 0
        for x in titles:
            if re.match(winTitle+' - v[0-9.]* - host: desktop',x[0]) and x[1] == 'QWidget':
                ...
                if is_opened == 1:
                    ...
                    return
        # no instance and create instance
